...
- **`story_design.md` 与 `crisis_arcs.yaml` 分工**(设计 Agent 生成两份文件时必须遵守, 避免互相抄一遍): `crisis_arcs.yaml` 的 `main_arc` 是"外部世界的心跳"——脚本化、按时间/触发条件推进的具体事件; `story_design.md` 是给 DM/主席用的**叙事透镜**, 装 `main_arc` 装不下的东西——跨越整局的判断框架、分支之间的转化条件("内阁若公开分裂, 同样动作会被解读为失控")、DM 该怎么拿捏节奏的教练笔记. 两者对同一个事件各自成文时, `story_design.md` 里对应段落要写清引用哪个 `main_arc` 事件 id, 不重复抄内容(时间线表格、事件正文都不许在两份文件里各写一遍).
...
- **文字表达** `text '<自然语言陈述>'`: 如 `text '国防部长与首相的矛盾公开化'`, 由 DM(LLM)对照当前局势判断真伪. 写成**可判定的陈述句**, 不写疑问句/程度模糊句; 同一分支组的多个事件应共用同一句陈述、一正一 `not`, 保证互斥(见示例).
...
**写作纪律(成本优化)**: 生成表达式时必须利用短路语义控制 LLM 判定频率——

- **能用时间判定表达的条件, 就不要用 `text`**: 时间比较是零成本的程序求值, `text` 每评估一次都要烧一次 LLM 调用;
- 时间与 `text` 混用时, **把时间比较放在 `text` 之前**(同一条 and/or 链内), 让程序可求值的部分先跑;
- 且**让排在前面的时间条件倾向于否决整个表达式**: and 链开头放一个大多数回合为 False 的"时间闸门"(如 `time >= 某事件 + 3h and text '…'`——闸门未开时 text 根本不会被评估), 使 `text` 判定只在真正到达时间窗口后的少数回合发生.

### 线性、分支与收敛的表达

- **线性链**: 后一事件的 trigger 相对前一事件定时(`time >= 前事件 + 2h`)——"上一拍发生后 2 小时进下一拍";
- **分支**: 同一时间窗口内的两个事件, condition 用同一句 text 陈述一正一反(`text 'X'` / `not text 'X'`), 互斥二选一;
- **收敛**: 无需特殊写法——后续事件用绝对时间, 或用 `or` 引用多个前驱, 天然汇合;
- **兜底**: 不需要"其他"分支——所有 condition 都不满足时就什么都不触发, DM 本来就有权即兴投放弧线外事件(弧线是参考, 不是脚本).
...
main_arc:
  - id: border_skirmish
    trigger: "time >= 2026-03-15T13:00:00+08:00"    # 绝对时间; condition 省略 = 到点必发
    content: |
      边境传来消息: 邻国军队在争议地区进行实弹演习, 双方边防部队发生对峙.
    default_scope: global

  - id: parliament_window
    trigger: "time >= border_skirmish + 2h"          # 线性链: 相对前一事件定时
    content: |
      议会就边境对峙召开紧急质询, 内阁必须有可公布的立场.
    default_scope: global

  - id: diplomatic_backchannel                       # 分支一(降级走向)
    trigger: "time >= parliament_window + 1h and time < 2026-03-16T09:00:00+08:00"
    condition: "text '内阁已通过外交渠道推动降级或与邻国实质接触'"
    content: |
      邻国通过秘密渠道释放缓和信号, 但国内鹰派强烈不满.
    default_scope: global

  - id: escalation_night_patrol                      # 分支二: 同一陈述取反, 与分支一互斥
...
- **不回喂模型上下文**: agent 感知 todo 靠对话历史里能看到自己之前的 `edit_todo` 调用、`edit_todo` 返回值里带回最新全文、或主动调 `check_todo`, 不在每步 system 前缀里自动注入(避免打穿 prompt 缓存, 见 03§7.4);
