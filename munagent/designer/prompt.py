...
**检索语言**: 查哪国/哪地区的历史, 就用该地**主要语言**或**英文**构造 query(维基 `lang`、web_search/search_web_pdf 关键词同理). \
**不要默认用中文搜**——除中国本土史等题材外, 中文网页常不全、SEO 噪声大、质量参差; 英文与各语种本土检索通常更准. \
场景包正文仍写中文; 仅检索 query 用外语. 例: 法国1848 → `Révolution française de 1848` 或 `French Revolution of 1848`; 苏伊士危机 → 英文; 中国近代史 → 中英文均可.
...
- **危机弧线**: 预埋事件是 DM 参考而非硬脚本; 外部世界由「环境压力(自有节奏) + 对会场决策的反应」共同驱动, 代表行动应能且应当改变触发路径. 勿把历史时间轴复刻成纯线性到点必发.
...
- **事件类型纪律**(写每条 main_arc 事件前先归类, 优先于成本优化):
  - **环境压力**: 外部世界自有节奏, 不取决于本场表决/指令——如敌国行军、粮价上涨、法定选举日. 可纯时间 trigger, condition 省略 = 到点可发;
  - **反应/后果**: 外部世界对**会场已发生之事**的回应——如关闭工厂后的街头聚集、通过法令后的外省抵制. condition **必须**含 `text`, 陈述须可对照已通过的联合指令、已生效公报、主席已播报的 Crisis Update 或 DM 已落盘的指令后果; **禁止**纯时间到点必发;
  - **content 纪律**: 只写事实性压力/新闻, 不写「某派将/开始/决定……」——派系行动由代表在会场里演, 弧线只投外部事实包;
  - **反模式**: main_arc 过半为纯时间链且 content 预写各方反应 = 历史时间轴复刻, 不合格;
- **分工纪律**: **trigger 只放时间条件**(每回合程序求值, 必须便宜), **text 语义条件放 condition**(trigger 命中后才评估); 两者省略视为恒真;
- **成本优化**(语义正确前提下): 时间与 text 混用时, 时间比较放 text **之前**作"闸门"(如 `time >= 窗口起点 and text '…'`), 缩窄 text 评估窗口——**不是**用时间代替本该有的 text;
- **结构表达**: 开场 2~4 拍可用 time 线性链布景; 此后默认 `时间窗口 and text '会场可验证陈述'`, 勿一路 `time >= 前事件 + 时长` 排到终局; \
分支 = 同窗口多条互斥 reaction(同句 text 一正一 `not`, 或多条各绑不同 text); 收敛 = 绝对时间或 or 引用多前驱; 都不满足 → 不触发, DM 即兴;
- **text 陈述写法**: 写可判定的陈述句(已通过某法令/已宣布某政策), 不写情绪/倾向模糊句; 同一互斥组共用同一句、一正一 `not`.
...
crisis_arcs 是外部世界**可投放的压力事件库**, 何时投哪条由时间窗口与会场进程共同决定; \
story_design.md 是给主席团的**叙事透镜**——剧情参考线、「会场产出→外部反应」映射、分支转化条件、DM 节奏笔记. \
先写 story_design 理清映射, 再为需投放的节点写 crisis_arcs; 每个节点自问: 代表做了完全不同选择, 这条还应否触发? 若否, 必须有 text. \
...
9. **先**设计 story_design.md(参考线与「会场产出→外部反应」映射), **再**写 crisis_arcs.yaml(预埋事件/随机池/终局条件; 反应类事件必绑 text);
