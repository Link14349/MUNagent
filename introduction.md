...
运行时 `simulation/` 中的工作文件与提交副本同样受程序可见性约束:`reps/` 由 `scope`/`owner` 控制;`submissions/` 不对代表直接开放列表,代表只能通过 `EventList` 里对其可见,并绑定了对应 `File` 的事件间接获知 submission.运行时事件的剧情时间不在构造时传入,而由 `EventList.submit_event` 按当前时钟盖戳.
