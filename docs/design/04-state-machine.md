...
阶段切换决策者是主席Agent(输入: 会场近期氛围摘要+待决动议+阶段预算余量; 输出schema见05), 人类导演可通过`human_control`事件强制切换, 优先级最高.

### ModeratedCaucus回合流程
1. 主席点名: 主席Agent从"举手席位+未发言席位"中选择下一位(输出`next_speaker`), 引擎附加**保底轮询**规则——连续K轮(默认K=会场人数)未发言的席位强制获得一次发言机会, 防饿死;
2. 被点名代表执行一次行动回合(可选动作: 发言/动议/写指令/pass, 见05的输出schema);
3. 动作产生对应事件; 若为动议, 主席即刻裁决(受理→切阶段或进Voting; 驳回→继续);
...
- **内部一律UTC**: 引擎只有一条时间轴——事件`story_time`、`takes_effect_at`、弧线触发`at`、manifest起止时间全部以UTC持久化(ISO带`Z`). 时序比较(生效排序/弧线触发/跨会场对齐)只在UTC轴上做, 双时区参与比较是bug温床;
