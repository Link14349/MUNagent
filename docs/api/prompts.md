...
| task | 触发时机 | 要点 |
|------|----------|------|
| `turn` | 被点名行动 | 阶段、故事时间, 引用 G 段 turn schema. 主持席额外提示可偏心; Unmod 末轮附 next_move 要求(`require_next_move=True`) |
...
- `build_turn_context(task, visible_events, phase, story_time, is_presiding, l2_summary, require_next_move)` — `visible_events` 应为纪元过滤后的列表
- `build_vote_context(task, visible_events, directive_title, story_time, directive_body, l2_summary)`
...
L4 由 `build_summarize_prompt(old_summary, new_events, level)` 生成, 含旧摘要、本期事件(摘要专用截断渲染)、压缩指令.
...
主席与 DM 有各自的 G 段常量, 不由 `build_delegate_g_global` 生成. 当前主席/DM 的 L2 摘要**已生成但未接入**其 context 构建(仅代表 `turn` 使用 L2).
