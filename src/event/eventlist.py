...
class VenueEventList:
    """单个会场的事件容器.

    各会场使用独立的事件列表与 pending 队列,便于按会场分线程推进时
    互不争用同一可变结构.事件 ``id`` 仅在本会场内唯一.
    """
    ...
    def __init__(self, venue_id: str) -> None:
        if not venue_id:
            raise ValueError("VenueEventList.venue_id 不能为空")
    ...
    def pending_event_ids(self) -> list[int]:
        """本会场仍为 PENDING 的事件 id(按入队顺序)."""
        return list(self.__pending_events)

    @property
    def pending_events(self) -> list[Event]:
        """本会场仍为 PENDING 的事件(按入队顺序)."""
        return [self.__events[event_id] for event_id in self.__pending_events]

    def submit_event(self, event: Event, *, time: datetime) -> None:
        """盖戳并登记到本会场容器;不负责通知 listener."""
        if event.venue != self.venue_id:
            raise ValueError(
                f"事件 venue={event.venue!r} 与容器 venue_id={self.venue_id!r} 不一致"
            )
    ...
    def _event_updated(self, event: Event) -> None:
        """事件离开 PENDING 时移出本会场 pending 队列."""
        if event.id is None:
        ...
        if (
            ...
            raise ValueError(
                ...
                f"{self.venue_id!r} 的事件容器,不能更新 pending"
            )
        if event.status == EventStatus.PENDING:
            raise ValueError(
...
class EventList:
    ...
    事件对象构造时 ``time`` / ``id`` 为 ``None``;``submit_event`` 用当前表时钟盖戳,
    并按 ``event.venue`` 写入对应 ``VenueEventList``(``id`` 在该会场内唯一).
    仍为 ``PENDING`` 的事件进入该会场的 pending 队列;``Event.status`` 离开
    ...
    __by_venue: dict[str, VenueEventList]
    __time: datetime
    ...
    def __init__(self, scenario: Scenario):
        ...
        if not scenario.venues:
            raise ValueError("场景无会场,无法创建分会场 EventList")
        seen: set[str] = set()
        self.__by_venue = {}
        for venue in scenario.venues:
            if not venue.id:
                raise ValueError("会场 id 不能为空,无法创建事件容器")
            if venue.id in seen:
                raise ValueError(f"重复的会场 ID: {venue.id}")
            seen.add(venue.id)
            self.__by_venue[venue.id] = VenueEventList(venue.id)

    def for_venue(self, venue_id: str) -> VenueEventList:
        """返回指定会场的事件容器."""
        try:
            return self.__by_venue[venue_id]
        except KeyError as exc:
            raise ValueError(
                f"未知会场 {venue_id!r},当前事件容器: {sorted(self.__by_venue)}"
            ) from exc
    ...
    @property
    def pending_event_ids(self) -> list[int]:
        """全会场 PENDING 事件的 id(按会场声明顺序再按入队顺序).

        多会场下 id 仅在各自会场内唯一,跨会场可能重复;需要消歧请用
        ``for_venue(venue_id).pending_event_ids`` 或 ``pending_events``.
        """
        return [event.id for event in self.pending_events if event.id is not None]

    @property
    def pending_events(self) -> list[Event]:
        """全会场仍为 PENDING 的事件(按会场声明顺序,再按各会场入队顺序)."""
        result: list[Event] = []
        for venue in self.scenario.venues:
            result.extend(self.__by_venue[venue.id].pending_events)
        return result
    ...
    def submit_event(self, event: Event):
        """将事件登记入对应会场容器,再通知匹配的 listener."""
        store = self.for_venue(event.venue)
        store.submit_event(event, time=self.__time)
        for listener_venue, listener in self.__listeners.get(event.type, ()):
            if listener_venue is None or listener_venue == event.venue:
                listener(event)

    def _event_updated(self, event: Event) -> None:
        """由 ``Event.status`` setter 在离开 PENDING 时回调;路由到对应会场容器.

        约定仅供 Event 调用,不作为对外 API.
        """
        self.for_venue(event.venue)._event_updated(event)
    ...
    def get_events(self, rep: str) -> list[Event]:
        ...
        ``__GOD__`` 返回全部会场事件(按会场声明顺序).
        """
        result: list[Event] = []
        for venue in self.scenario.venues:
            result.extend(self.__by_venue[venue.id].get_events(rep))
        return result
