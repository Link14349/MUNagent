...
class File:
    ...
    def __init__(
        ...
        *,
        ...
        self.__scope = set(scope) if scope is not None else set(self.__owner)
    ...
    def _ensure_owners_in_scope(self) -> None:
        missing = self.__owner - self.__scope
        if missing:
    ...
    def add_owner(self, actor: str, obj: set[str]) -> None:
        ...
        with self._filesystem_lock():
            ...
            for identity in newcomers:
                if identity not in self.__scope:
                    raise PermissionError(
                        f"不能将 {identity!r} 设为 owner:其不在文件 {self.path} 的 scope "
                        f"(当前 scope={sorted(self.__scope)})"
                    )
...
class FileSystem:
    ...
    def _persist_file(self, file: File) -> None:
        with self._lock:
            _atomic_write_text(file.path, file._raw_content())
            self._save_manifest()
